...
export function DeleteFood({ id }) {
  ...
  const { foodsRenderStore } = useRenderStore(); // For rendering updates
  ...
  const handleDelete = async () => {
    setLoading(true);
    try {
      const response = await $api.delete(`/foods/delete/${id}`); // Using the specified API endpoint
      if (response.status === 200) {
        sweetAlert("Food establishment deleted successfully", "success");
        setOpen(false);
        foodsRenderStore(); // Call the store method to refresh the data
      } else {
